...
// 先求bounding box，新写的删除笔迹bounding box对所有的字的bounding box求交叉面积，最大交叉面积即是要新增的字符。
//1122
var pointX = 0;
var isShow = false;
var tmpStr = "";
...
function character(){
	this.charlist = []; // 字数组
	this.boudingbox = []; // xmin, xmax, ymin, ymax
	this.line = 0; // 属于哪一行
}
//画布创建与笔迹位置坐标存储
createDrawArea();
...
getRecogResult = function(){
	var box = getBoundingBox(strokelist); 
	replace_idx = box_list.length;
	var flag = 0;
	for(char=0; char < box_list.length; char++)
	{
		if(IOU(box_list[char], box) > 0.5){
			replace_idx = char;
			flag = 1;
			break;
		}
	...
	$.ajax({
		...
		success : function(str){
			...
			if(str == ('w')){
				...
				if (flag == 1){ // 与某个字符高度重合，则删除这个字符
					console.log("replace_idx " + replace_idx);
					reviseContext(canvasChosen, replace_idx);
					reviseMark(ctx, replace_idx);	
				}
				else{  // 不重合 则插入
					...
					// xmiddle = box.x + box.w / 2;
					// insert_idx = box_list.length;
					// for(char=0; char < box_list.length; char++)
					// {
					// 	charmiddle = box_list[char].x + box_list[char].w /2;
					// 	if(xmiddle < charmiddle){
					// 		insert_idx = char;
					// 		break;
					// 	}
					// }
...
getRecogResultElse = function(){
	...
	for(char=0; char < box_list.length; char++)
	{
		if(IOU(box_list[char], box) > 0.5){
			replace_idx = char;
			flag = 1;
			maxIou = IOU(box_list[char], box);
			break;
		}
	}
	console.log("识别之前: ");
	console.log(box_list.length, charlist.length);
	console.log("flag " + flag);
		...
		success : function(str){
			...
			if(str == ('ｗ')){
				...
				if (flag == 1){ // 与某个字符高度重合，则删除这个字符
					...
					addContext_new(canvasChosen,insert_idx, chem);
					addMark_new(ctx, insert_idx, box, strokelist);
				}
				strokelist = [];
				if(!isTable){
					var res = result.length;
...
// wx 2020.3.10
function getBoundingBox(strokelist){
	var touches = strokelist.length;
	var xlist = [];
	var ylist = [];
	for (var i=0; i< touches; i++) // 笔画数组
	{
		xlist.push(strokelist[i].x);
		ylist.push(strokelist[i].y);
	}
	console.log("xlist ", xlist);
	console.log("ylist ", ylist);
	var xleft, xright, ytop, ybottom;
	...
	w = xright - xleft;
	h = ybottom - ytop;
	box = {"x":xleft, "y":ytop, "w":w, "h":h};
	console.log(box);
	ctx.rect(xleft, ytop, w, h);
	ctx.stroke();
	return box;
}
...
function IOU(a, b){
	var area_a = a.w * a.h;
	var area_b = b.w * b.h;
	var w = Math.min(b.x+b.w,a.x+a.w) - Math.max(a.x, b.x);
	var h = Math.min(b.y+b.h,a.y+a.h) - Math.max(a.y,b.y);
	if (w<=0 || h <=0){
		console.log("IOU " +0);
		return 0;
	}
	area_c = w*h;
	console.log("IOU " + area_c / (area_a + area_b - area_c));
	return area_c / (area_a + area_b - area_c);
}

function reviseContext(canvasChosen, replace_idx){
	if(!isTable){							
		if(isNextLine){
			tmpStr = "";
			lineNum++;
		...
		tmpStr = thisline.innerHTML;
		if(replace_idx == tmpStr.length){
			tmpStr = tmpStr.slice(0, tmpStr.length - 1);
		}
		else{
			tmpStr = tmpStr.slice(0, replace_idx) + tmpStr.slice(replace_idx + 1);
		}
		// 12.31注释
		// if (tmpStr.includes(' ')){			
		// 	thisline.innerHTML="";
...
function reviseMark(context, replace_idx)
{	
	context.clear();
	//charlist.pop();
	//box_list.pop();
	console.log(charlist.length + " " + box_list.length);
	if(replace_idx == charlist.length){
		charlist = charlist.slice(0, charlist.length - 1);
		box_list = box_list.slice(0, box_list.length - 1);
	}
	else{
		var chartmp1 = charlist.slice(0, replace_idx);
		var chartmp2 = charlist.slice(replace_idx + 1);
		charlist = chartmp1.concat(chartmp2);
		var boxtmp1 = box_list.slice(0, replace_idx);
		var boxtmp2 = box_list.slice(replace_idx+1);
		box_list = boxtmp1.concat(boxtmp2);
	}
	console.log(charlist.length + " " + box_list.length);
	for (var char=0; char<charlist.length; char++)
	{
...
// 2020.5.5
function addContext_new(canvasChosen, insert_idx, chem){
	if(!isTable){							
		if(isNextLine){
			tmpStr = "";
			lineNum++;
		...
		tmpStr = thisline.innerHTML;
		console.log("tmpStr_before " + tmpStr);
		if(insert_idx == tmpStr.length){
			tmpStr = tmpStr + chem;
		}
		else{
			tmpStr = tmpStr.slice(0, insert_idx) + chem +  tmpStr.slice(insert_idx);

		}
		
		console.log("tmpStr_after " + tmpStr);
		thisline.innerHTML = tmpStr;
		midOfView();
	}
	else{	
...
function addMark_new(context, insert_idx, box, stroke)
{	
	console.log("addMark_new 开始");
	context.clear();
	console.log("比较插入索引与长度: "  + insert_idx + " " + charlist.length);
	if(insert_idx == charlist.length){
		charlist.push(stroke);
		box_list.push(box);
	}
	else{
		var chartmp1 = charlist.slice(0, insert_idx);
		var chartmp2 = charlist.slice(insert_idx);
		chartmp1.push(stroke);
		charlist = chartmp1.concat(chartmp2);
		var boxtmp1 = box_list.slice(0, insert_idx);
		boxtmp1.push(box);
		var boxtmp2 = box_list.slice(insert_idx);
		box_list = boxtmp1.concat(boxtmp2);
	}
	console.log(charlist.length + " " + box_list.length);
	for (var char=0; char<charlist.length; char++)
